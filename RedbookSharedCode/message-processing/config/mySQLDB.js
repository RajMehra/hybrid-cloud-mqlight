...
var _dbNameBase = "cloudant-smart-payments-api"
var env = process.env.ENV || "test";
var dbHost = process.env.DBHOST || "129.41.143.50";
...
function getVotesbyChoice(voterChoice, callback){
		...
		var qryString;
		if (voterChoice){
			qryString = "select referenceNumber,name,email,phone,voterChoice,votetimeStamp from VOTES where voterChoice = '" + voterChoice + "' order by votetimeStamp";

		}
		else {
			qryString = "select referenceNumber,name,email,phone,voterChoice,votetimeStamp from VOTES order by votetimeStamp";			
		}
